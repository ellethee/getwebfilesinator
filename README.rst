GetWebFilesInator
=================

|build-status| |docs|

Description
-----------

GetWebFilesInator (gwfi) is a simple file downloader to keep your javascript
and css libraries up to date. Is not perfect but is easily configurable and
for simplest tasks can be more suitable than a real dependency manager.

All you need is to know were your libraries are and a yaml or json configuration
file.

**Please See documentation at** `Read the docs`_

.. _Read the docs: http://getwebfilesinator.readthedocs.io/en/latest/
